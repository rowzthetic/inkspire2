      ...
      <aside className="dashboard-sidebar">
        <div>
          ...
          <nav className="nav-menu">
            ...
            {hasCompleted && <NavButton icon={<Heart size={20} />} label="Healing" active={activeTab === 'healing'} onClick={() => handleTabChange('healing')} />}
...
const RevenueSection = () => {
  ...
  const [stats, setStats] = useState({ total_revenue: 0, order_count: 0, paidDeposits: 0, pendingDeposits: 0, refundedDeposits: 0 });
  ...
  const handleDownloadPDF = () => {
    try {
      ...
      doc.text(String(transactions.length), 150, 78);
      ...
      autoTable(doc, {
        ...
        body: transactions.map(t => [
          ...
          t.is_refunded ? 'Refund' : 'Booking Deposit',
  ...
  useEffect(() => {
    const fetchAllFinancials = async () => {
      ...
      try {
        ...
        const paidDeps = appts.filter(a => a.is_deposit_paid && a.status !== 'cancelled' && !a.is_refunded).reduce((sum, a) => sum + parseFloat(a.deposit_amount || 0), 0);
        const pendingDeps = appts.filter(a => a.status === 'confirmed' && !a.is_deposit_paid).reduce((sum, a) => sum + parseFloat(a.deposit_amount || 0), 0);
        const refundedDeps = appts.filter(a => a.is_refunded).reduce((sum, a) => sum + parseFloat(a.deposit_amount || 0), 0);

        setStats({ total_revenue: shopData.total_revenue || 0, order_count: shopData.order_count || 0, paidDeposits: paidDeps, pendingDeposits: pendingDeps, refundedDeposits: refundedDeps });
        setTransactions(appts.filter(a => a.is_deposit_paid || a.is_refunded));
  ...
  return (
    <div className="schedule-card">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '30px' }}>
        ...
        <StatCard label="Refunded Deposits" value={`$${stats.refundedDeposits.toFixed(2)}`} icon={<AlertCircle size={24} color="#ef4444" />} color="rgba(239, 68, 68, 0.1)" />
      </div>
              ...
              {transactions.map(t => (
                <tr key={t.id} style={{ borderBottom: '1px solid #27272a' }}>
                  ...
                  <td style={{ padding: '12px 16px' }}>
                    {t.is_refunded ? (
                      <span style={{ backgroundColor: 'rgba(239, 68, 68, 0.15)', color: '#f87171', padding: '4px 8px', borderRadius: '4px', fontSize: '12px', fontWeight: 'bold' }}>Refunded</span>
                    ) : (
                      <span style={{ backgroundColor: 'rgba(16, 185, 129, 0.15)', color: '#34d399', padding: '4px 8px', borderRadius: '4px', fontSize: '12px', fontWeight: 'bold' }}>Paid</span>
                    )}
                  </td>
                  <td style={{ padding: '12px 16px', textAlign: 'right', fontWeight: 'bold', color: t.is_refunded ? '#ef4444' : '#10b981' }}>{t.is_refunded ? `-${t.deposit_amount}` : `+${t.deposit_amount}`}</td>
                </tr>
              ))}
            </tbody>
          </table>
