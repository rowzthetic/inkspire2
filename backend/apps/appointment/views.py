...
#         # Don't copy request.data - work with it directly for file uploads
#         data = {}
...
#         # 👇 FULL STRIPE REFUND LOGIC
#         if appointment.is_deposit_paid and hasattr(appointment, 'stripe_payment_intent_id') and appointment.stripe_payment_intent_id:
#             try:
#                 # By passing just the payment intent, Stripe issues a 100% full refund
...
#         appointment.save()
#         return Response({"message": "Appointment cancelled and refunded successfully."}, status=200)
...
#         # Only the assigned artist can manage this appointment
...
#             # Allow updating price quote for pending/confirmed appointments
#             if price_quote:
#                 appointment.price_quote = price_quote
#             if deposit_amount:
#                 appointment.deposit_amount = deposit_amount
...
#             return Response(
#                 {
#                     "error": "Invalid action. Use 'accept', 'decline', or 'update_quote'."
#                 },
#                 status=400,
#             )
...
#         # Security: Only the customer who booked it can pay
...
#         # Default to $50 deposit if the artist didn't specify one
...
#                         'price_data': {
#                             'currency': 'usd', 
#                             'unit_amount': int(deposit_amount * 100), # Stripe expects cents
#                             'product_data': {
...
#                 # Mark as paid in the database AND save the payment intent for future refunds
#                 appointment.is_deposit_paid = True
                
#                 if hasattr(appointment, 'stripe_payment_intent_id'):
#                     appointment.stripe_payment_intent_id = session.payment_intent # Saves Stripe Payment Intent ID
                
#                 appointment.save()
                
#                 return Response({"message": "Payment verified and deposit marked as paid."})
#             else:
...
from .models import Appointment, ArtistAvailability, HealingNote
from .serializers import (
    AppointmentSerializer,
    ArtistAvailabilitySerializer,
)
...
class HealingNotesView(APIView):
    ...
    def get(self, request, appointment_id):
        ...
        notes = {str(n.day): n.note for n in HealingNote.objects.filter(appointment=appointment)}
    ...
    def put(self, request, appointment_id):
        ...
        HealingNote.objects.bulk_create([
            HealingNote(appointment=appointment, day=int(day), note=str(text).strip())
